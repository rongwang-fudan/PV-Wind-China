...
for i=1:numpowerunit
    i2=IX(i);
    powerunit_IX(i,1)=i2;
    optpowerunit_IX(i,1:40)=optpowerunit(i2,1:40); %
    lines_IX_IX(i,1:15)=lines_IX(i2,1:15); % 
end

%%
load('H:\China C neutrality\ANS\CO2_mecha.mat'); % 
load('H:\China C neutrality\ANS\B_mecha.mat'); % 
load('H:\China C neutrality\ANS\LCOEE_mecha.mat'); % 
CO2_mecha = CO2all_c_utilize_trans_storage;
B_mecha = B_utilize_trans_storage;
LCOE_mecha = LCOEE_all_utilize_trans_storage;

load('H:\China C neutrality\ANS\CO2_Battery.mat'); % 
load('H:\China C neutrality\ANS\B_Battery.mat'); %
load('H:\China C neutrality\ANS\LCOEE_Battery.mat'); % 
...
for i = 1:3844
    if LCOE_mecha(i,1)<LCOE_Battery(i,1)
        cho(i,1)=1; % mechanical storage
        B(i,1) = B_mecha(i,1);
        CO2(i,1) = CO2_mecha(i,1);
    end
    if LCOE_mecha(i,1)>LCOE_Battery(i,1)
        cho(i,1)=-1; % chemical storage
        B(i,1) = B_Battery(i,1);
        CO2(i,1) = CO2_Battery(i,1);
    end
    if LCOE_mecha(i,1)==LCOE_Battery(i,1)
        cho(i,1)=0; % no storage
        B(i,1) = B_Battery(i,1);
        CO2(i,1) = CO2_Battery(i,1);
    end
end
