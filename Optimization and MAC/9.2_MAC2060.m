...
load('H:\China C neutrality\ANS\utilize_ratio2060_trans_plant_alone_xz.mat')  % power use efficiency with inter-regional electric transport but without storage
load('H:\China C neutrality\ANS\utilize_ratio2060_trans_storage_plant_alone_xz.mat')  % power use efficiency with inter-regional electric transport and storage
...
load('H:\China C neutrality\ANS\cost_trans_IX2_xz.mat')% transmission cost
load('H:\China C neutrality\ANS\CP_trans_IX2_xz.mat')%
load('H:\China C neutrality\ANS\cho_xz.mat'); %
...
for i = 1:1:4
    [m,n] = find(unitmin==i);
    learnprice_sto_stage_p(m,:) = rIp(i);
    learnprice_sto_stage_e(m,:) = rIe(i);
end
...
learnprice_trans1(1,1) =1;
[m,n] = find(unitmin==1);
learnprice_trans(m,1) = learnprice_trans1(1,1);
cost_trans_LR(m,1) = cost_trans_IX(m,1)*learnprice_trans1(1,1);
for i = 2:1:4
    [m,n] = find(unitmin<=i-1);
    CPP = sum(CP_trans_IX(m))+CP0_PV;
    learnprice_trans1(i,1) = (CPP./CP0_PV).^(log(1- LR_PV2)/log(2));
    [m,n] = find(unitmin==i);
    learnprice_trans(m,1) = learnprice_trans1(i,1);
    cost_trans_LR(m,1) = cost_trans_IX(m,1)*learnprice_trans1(i,1);
end
clear learnprice_trans1
...
powerunit_num = [powerunit_num_IX_PV;powerunit_num_IX_onshorewind;powerunit_num_IX_offshorewind];
...
for i=1:numpowerunit
    ...
    powerunit_num_IX(i,1:4)=powerunit_num(i2,1:4); % lat lon
end
...
CP_cum(:,1) = cumsum(CP_PV_alone)/10^6; % GW
CP_cum(:,2) = cumsum(CP_onshorewind_alone)/10^6; % GW
CP_cum(:,3) = cumsum(CP_offshorewind_alone)/10^6; % GW
...
%%
learnprice_PV_stage = zeros(3844,1);
learnprice_PV_stage_2 = zeros(3844,1);
learnprice_onshorewind_stage = zeros(3844,1);
learnprice_onshorewind_stage_2 = zeros(3844,1);
learnprice_offshorewind_stage = zeros(3844,1);
learnprice_offshorewind_stage_2 = zeros(3844,1);
for i = 1
    [m1,n] = find(unitmin==i & optpowerunit_IX(:,35)==1);
    learnprice_PV_stage(m1,1) = 1;
    learnprice_PV_stage_2(m1,1) = 1;
    [m1,n] = find(unitmin==i & optpowerunit_IX(:,35)==2);
    learnprice_onshorewind_stage(m1,1) = 1;
    learnprice_onshorewind_stage_2(m1,1) = 1;
    [m1,n] = find(unitmin==i & optpowerunit_IX(:,35)==3);
    learnprice_offshorewind_stage(m1,1) = 1;
    learnprice_offshorewind_stage_2(m1,1) = 1;
end
for i = 2:1:4
    [m,n] = find(unitmin==i-1 & optpowerunit_IX(:,35)==1);
    [m1,n] = find(unitmin==i & optpowerunit_IX(:,35)==1);
    learnprice_PV_stage(m1,1) = learnprice_PV(m(end));
    learnprice_PV_stage_2(m1,1) = learnprice_PV_2(m(end));
    [m,n] = find(unitmin==i-1 & optpowerunit_IX(:,35)==2);
    [m1,n] = find(unitmin==i & optpowerunit_IX(:,35)==2);
    learnprice_onshorewind_stage(m1,1) = learnprice_onshorewind(m(end));
    learnprice_onshorewind_stage_2(m1,1) = learnprice_onshorewind_2(m(end));
    [m,n] = find(unitmin==i-1 & optpowerunit_IX(:,35)==3);
    [m1,n] = find(unitmin==i & optpowerunit_IX(:,35)==3);
    learnprice_offshorewind_stage(m1,1) = learnprice_offshorewind(m(end));
    learnprice_offshorewind_stage_2(m1,1) = learnprice_offshorewind_2(m(end));
end
...
lcoee_PV_noUHV(:,3) = cost_PV_alone(:,13)./disrate_PV(4,1).*learnprice_PV_2; % cost million USD
lcoee_PV_UHV(:,3) = cost_PV_trans(:,1).*learnprice_trans; % cost million USD
...
lcoee_onshorewind_noUHV(:,3) = cost_onshorewind_alone(:,13)./disrate_wind(4,1).*learnprice_onshorewind_2; % cost million USD
lcoee_onshorewind_UHV(:,3) = cost_onshorewind_trans(:,1).*learnprice_trans; % cost million USD
% 5  connection to national grid
...
lcoee_offshorewind_noUHV(:,3) = (cost_offshorewind_alone(:,6)./disrate_wind(4,1)-module_price_offshore.*CP_offshorewind_alone(:,1)).*learnprice_offshorewind_2;
lcoee_offshorewind_UHV(:,3) = cost_offshorewind_trans(:,1).*learnprice_trans ; % cost million USD
% 9  O&M cost
...
lcoeeall_utilize_trans_storage2(sub2ind(size(lcoeeall_utilize_trans_storage2), m, n))= lcoee_offshorewind(sub2ind(size(lcoee_offshorewind), m, n));
lcoeeall_utilize_trans_storage2(sub2ind(size(lcoeeall_utilize_trans_storage2), m, 2*ones(size(m,1),1)))= lcoee_offshorewind_noUHV(sub2ind(size(lcoee_offshorewind_noUHV), m, 3*ones(size(m,1),1)));
...
lcoee_noUHV(sub2ind(size(lcoee_noUHV), m, 3*ones(size(m,1),1)))= lcoee_offshorewind_noUHV(sub2ind(size(lcoee_offshorewind_noUHV), m, 3*ones(size(m,1),1)));
aa =  lcoee_offshorewind(:,2) - lcoee_offshorewind_noUHV(:,3);
lcoeeall_utilize_trans_storage2(sub2ind(size(lcoeeall_utilize_trans_storage2), m, 12*ones(size(m,1),1)))= aa(sub2ind(size(aa), m, ones(size(m,1),1)));
lcoeeall_utilize_trans_storage2(sub2ind(size(lcoeeall_utilize_trans_storage2), m, 6*ones(size(m,1),1)))= lcoee_offshorewind_utilize_trans_storage(sub2ind(size(lcoee_offshorewind_utilize_trans_storage), m, ones(size(m,1),1)));
...
lcoeeall_utilize_trans_storageaa(:,1:9) = lcoeeall_utilize_trans_storage(:,1:9);
lcoeeall_utilize_trans_storageaa(:,10) = Cost_storage;
lcoeeall_utilize_trans_storageaa(:,11) = Cost_mechanical1;
%%
A = sum(lcoeeall_utilize_trans_storage(:,1:9),2)+Cost_storage+Cost_mechanical1;
B = phhall_utilize_trans_storage(:,1)./degrat40yr_offshorewind(4,1);
[b,ix1]=sort(LCOEE_all_utilize_trans_storage);
...
for i = 1:1:4
    ...
    a1 = A(m);
    b1 = B(m);
    c = LCOEE_all_utilize_trans_storage(m);
%     [a2,ix1]=sort(c,'descend');
    [a2,ix1]=sort(c);
    b2 = b1(ix1);
    a2 = a1(ix1);
    a_all = [a_all;a2];
...
lcoeeall_utilize_trans_storage2(:,11) = Cost_mechanical1;
...
CO2_all_utilize_trans_storage2 = CO2_PV_all_utilize_trans_storage; % Mton CO2
...
CO2_all_utilize_trans_storage2(sub2ind(size(CO2_all_utilize_trans_storage2), m, n))= CO2_onshorewind_all_utilize_trans_storage(sub2ind(size(CO2_onshorewind_all_utilize_trans_storage), m, n));
