...
cost_trans_LR(1:unitmin(2,1),1) = cost_trans_IX(1:unitmin(2,1),1);
cost_trans_LR(unitmin(2,1)+1:unitmin(3,1),1) = cost_trans_IX(unitmin(2,1)+1:unitmin(3,1),1)*learnprice_trans(unitmin(2,1));
cost_trans_LR(unitmin(3,1)+1:unitmin(4,1),1) = cost_trans_IX(unitmin(3,1)+1:unitmin(4,1),1)*learnprice_trans(unitmin(3,1));
cost_trans_LR(unitmin(4,1)+1:unitmin(5,1),1) = cost_trans_IX(unitmin(4,1)+1:unitmin(5,1),1)*learnprice_trans(unitmin(4,1));
...
save('H:\China C neutrality\ANS\LCOEE_STO_P2G_xz.mat','LCOEE_all_utilize_trans_storage'); 
...
save('H:\China C neutrality\ANS\rp_xz.mat','rp'); % 
...
clear Index2
clear Index3
save('H:\China C neutrality\ANS\ixx_2_xz.mat','ixx_2'); % 
...
save('H:\China C neutrality\ANS\LCOE_B_pv_xz.mat','LCOE_B_pv'); % USD2019/kWh
save('H:\China C neutrality\ANS\LCOE_B_ons_xz.mat','LCOE_B_ons'); % USD2019/kWh
save('H:\China C neutrality\ANS\LCOE_B_off_xz.mat','LCOE_B_off'); % USD2019/kWh
...
save('H:\China C neutrality\ANS\CO2_xz.mat','CO2all_c_utilize_trans_storage'); % 
save('H:\China C neutrality\ANS\B_xz.mat','B_utilize_trans_storage'); %

income_year = -(sum(lcoeeall_utilize_trans_storage(:,1:9),2)+Cost_storage+Cost_mechanical1-phhall_utilize_trans_storage(:,1).*EP*10^3)./degrat40yr_PV(4,1);
save('H:\China C neutrality\ANS\income_year_xz.mat','income_year'); % 
...
save('H:\China C neutrality\ANS\income_year_carbonpri_xz.mat','income_year_carbonpri'); % 

initial_cost = sum(lcoeeall_utilize_trans_storage(:,1:8),2)-lcoee_UHV(:,3);
save('H:\China C neutrality\ANS\initial_cost_xz.mat','initial_cost'); % 

UHV_sto_cost = lcoee_UHV(:,3)+Cost_storage+Cost_mechanical1;
...
MAC_mean = sum(sum(CO2all_c_utilize_trans_storage_IX2.*B_utilize_trans_storage_IX2))/sum(sum(CO2all_c_utilize_trans_storage_IX2))
CO2_max = -sum(sum(CO2all_c_utilize_trans_storage_IX2))/1000
Ini_per = sum(sum(initial_cost))/3844/1000

initial_cost_IX = initial_cost(IX,1);
unitmin_id_IX = unitmin_id(IX,1);
OM_IX = lcoeeall_utilize_trans_storage(IX,9)
...
UHV_sto_cost_IX = UHV_sto_cost(IX,1);
a_UHV_sto_cost_MAC_less0 = sum(UHV_sto_cost_IX(m))/1000;

[~,Index] = min(abs(CP_cumsum_all-2));
(sum(initial_cost_IX(1:Index))/1000+sum(UHV_sto_cost_IX(1:Index))/1000+sum(OM_IX(1:Index))/1000)/40

[~,Index] = min(abs(CP_cumsum_all-4));
-sum(CO2all_c_utilize_trans_storage_IX2(1:Index))/1000
%%
...
save('H:\China C neutrality\ANS\CO2_STO_P2G_xz.mat','CO2all2_c22_utilize_trans_storage_IX2'); % 
save('H:\China C neutrality\ANS\B_STO_P2G_xz.mat','B22_utilize_trans_storage_IX2'); % 
...
%%
storage_starty = [0:25:250]';
[~, index]=min(abs(CO2all2_c22_utilize_trans_storage_IX2_central/10^3-8));
...
plot(ones(11,1)*(-CO2all_c2_utilize_trans_storage_IX2(Index1,1)./10^3),storage_starty) 
