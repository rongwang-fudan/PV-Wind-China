...
tic
clear;
load('H:\China C neutrality\Data\SR_12to18_day.mat'); % SR_12to18_day(4800x1950)
SR_12to18_day(find(isnan(SR_12to18_day)==1))=0;
% SOCIAL-ECONOMY 1
load('H:\China C neutrality\Data\GDP_CN2015_120.mat'); % GDP_CN2015_120(4800x1950) ten thousand yuan
load('H:\China C neutrality\Data\POP_CN2020.mat'); % POP_CN2020(4800x1950) capita
...
% ENERGY Production 2
load('H:\China C neutrality\Data\CP_PV_120_all.mat'); % CP_PV_120_all(4800x1950)
% Capacity power of PV KW
load('H:\China C neutrality\Data\Ph_12to18_day.mat'); % Ph_12to18_day(4800x1950)
...
load('H:\China C neutrality\PV_power potential\ANS_PV1\unitid_all.dat','-mat');
...
for i=1:size(powerunit,1)
    ...
    for j=1:zmax
        ...
        idx2=find(z==j);
        idx2_all=find(z_all==j);
        
        if size(idx2,1)>0
            for k=1:size(idx2_all,1)
                x=xnet(idx_all(idx2_all(k)));
                y=ynet(idx_all(idx2_all(k)));
...
%                     id=id+1;
                    ...
                    costs(id,6)=trascost+costs(id,6); % cost of substation million RMB
                    SR2(id,1) = SR2(id-1,1);
                    SR2(id,2) = SR2(id-1,2);                    
                end
...
save('H:\China C neutrality\PV_power potential\ANS_PV1\costunits01_fig1b.dat','costs');
